...
import { getStudentDashboardData, getEducatorDashboardData } from '../../../../utils/api';
    ...
    const fetchData = async () => {
      try {
        ...
        setDashboardData({
          ...
          performanceTrendData,
          subjectWiseData,
          subjectWiseDataMapping: data.subjectWiseDataMapping || [],
          overallTestMarks,
          isLoading: false,
          error: null,
        });
...
// Hook for fetching and managing educator dashboard data
export const useEducatorDashboardData = () => {
  // State to hold the educator dashboard data and loading/error status
  const [dashboardData, setDashboardData] = useState({
    summaryCardsData: [],
    keyInsightsData: {},
    isLoading: true,
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch educator dashboard data from the API
        ...
        // Update the dashboard data state with the fetched data
        setDashboardData({
          summaryCardsData: data.summaryCardsData || [],
          keyInsightsData: data.keyInsightsData || {},
          isLoading: false,
