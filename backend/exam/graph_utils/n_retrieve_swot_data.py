...
def best_topic_analysis(kg_manager, test_name):
    ...
    best_topics = df_topic_metrics.sort_values(by=["WeightedScore", "ImprovementRate"], ascending=False).groupby("Subject").head(10)
    subject_dfs = {}
...
def low_retention_rate_analysis(kg_manager, test_name):
    ...
    low_retention = filtered.sort_values(by=["Accuracy", "Total"], ascending=[True, False]).groupby("Subject").head(10)
    subject_dfs = {}
...
def missed_opportunities_analysis(kg_manager, test_name):
    ...
    df_topic_metrics = pd.DataFrame(topic_metrics)
    missed = df_topic_metrics.sort_values(by=["WeightedScore", "ImprovementRate"], ascending=[False, True]).groupby("Subject").head(10)
    subject_dfs = {}
...
def most_challenging_topic_analysis(kg_manager, test_name):
    ...
    df_topic_metrics = pd.DataFrame(topic_metrics)
    challenging = df_topic_metrics.sort_values(by=["WeightedScore", "ImprovementRate"], ascending=True).groupby("Subject").head(10)
    subject_dfs = {}
...
def weakness_on_high_impact_analysis(kg_manager, test_name):
    ...
    df_topic_metrics = pd.DataFrame(topic_metrics)
    selected = df_topic_metrics.sort_values(by=["Total", "Accuracy"], ascending=[False, True]).groupby("Subject").head(10)
    subject_dfs = {}
