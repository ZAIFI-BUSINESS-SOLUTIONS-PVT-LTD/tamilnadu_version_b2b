...
import { generateTeacherSelfPdfReport, fetchAvailableSwotTests_InstitutionEducator, fetchInstitutionEducators } from '../../../utils/api.js';
...
export const TeacherReportModal = ({ onClose }) => {
  ...
  useEffect(() => {
    const loadEducators = async () => {
      try {
        ...
        // expect data.educators or array; normalize
        const list = Array.isArray(data) ? data : (Array.isArray(data?.educators) ? data.educators : []);
        setEducators(list);
        if (list.length) {
          setSelectedEducatorId(String(list[0].id ?? list[0].educator_id ?? ''));
        }
      } catch (err) {
  ...
  // Fetch available tests for the selected educator
  useEffect(() => {
    const fetchTests = async () => {
      setLoading(true);
      try {
          const edId = selectedEducatorId;
          const tests = edId ? await fetchAvailableSwotTests_InstitutionEducator(edId) : [];
    ...
    fetchTests();
    // set classId from selected educator when educator changes
    if (selectedEducatorId && educators.length) {
