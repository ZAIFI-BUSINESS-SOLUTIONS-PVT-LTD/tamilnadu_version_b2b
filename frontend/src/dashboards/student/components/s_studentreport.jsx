...
import { generateStudentSelfPdfReport, fetchAvailableSwotTests } from '../../../utils/api';
import { Button } from '../../../components/ui/button.jsx';
import { Card, CardContent, CardHeader, CardTitle } from '../../../components/ui/card.jsx';

// Modal component for students to download their individual reports
export const StudentReportModal = ({ onClose }) => {
  const [selectedTest, setSelectedTest] = useState('Overall');
  const [availableTests, setAvailableTests] = useState([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  // Fetch available tests for the student on mount
  useEffect(() => {
    const fetchTests = async () => {
      setLoading(true);
      try {
        const tests = await fetchAvailableSwotTests();
        // Format: [1,2,3] => ['Overall', 'Test 1', 'Test 2', ...]
        const uniqueTests = [...new Set(tests)].filter((num) => num !== 0);
        const testsList = ['Overall', ...uniqueTests.map((num) => `Test ${num}`)];
        setAvailableTests(testsList);
        // Set default to 'Overall' if available
        if (testsList.includes('Overall')) {
          setSelectedTest('Overall');
        }
      } catch (err) {
  ...
  // Prepare options for the test selection dropdown
  const testOptions = [
  ...
  const handleDownload = async () => {
    ...
    setError(null);
    try {
      const blob = await generateStudentSelfPdfReport(selectedTest);
      if (blob && blob.size > 0) {
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `inzighted_student_report_${selectedTest}.pdf`;
